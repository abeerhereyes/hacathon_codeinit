...
def decryptByPlayfairCipher(strr, key):
    # Function to call decrypt
    ks = len(key)
    key = removeSpaces(toLowerCase(key))
    strr = removeSpaces(toLowerCase(strr))
    keyT = generateKeyTable(key)
    return decrypt(strr, keyT)

global strr
def brain1(input_word):
    ...
    key = "Monarchy"
        ...
        # print("Plain text: ", input_word)

        # encrypt using Playfair Cipher
    strr = decryptByPlayfairCipher(input_word, key)
    print(strr)

        # Decrypted text
    print("Deciphered text: ", strr)
